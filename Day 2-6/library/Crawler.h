...
private:
    ...
    char *wgetFileDownload(const char *url,
                           const char *path); // function to fetch a url
    ...
    void createLogFile(const char *url,
                       const char *path,
                       char *mostFrequentWord); // log file containing
    int isKeywordPresentinFile(const char *allData,
                               const char *keyword);          // used for finding if keyword is present or not
    void uniqueUrlLogFile(const char *url, const char *path); // create unique url log file
    char **readHtmlUrls(const char *allData, const char *url,
                        int maxFoundPerPage); // find all Html or /(at end) urls
    void fileGetDfs(char *url,
                    const char *path,
                    int maxDepthCount,
                    int maxFoundPerPage); // recursively download html file -> read it -> extract all html anchor tag links -> again repeat until maxDepthCount comes
    char *most_frequent_word(const char *text,
                             const char **stopwords,
                             int stopcount); // find all most frequent words
